...
def _load_metric_config_with_override(config_dir: Path):
    """Load metric_config, preferring the provided config_dir when supported."""
    # Try common call shapes without breaking older signatures.
    try:
        return load_metric_config(config_dir=config_dir)  # type: ignore[arg-type]
    except TypeError:
        pass
    try:
        return load_metric_config(config_dir)  # type: ignore[misc]
    except TypeError:
        pass
    # Fall back to the default behaviour inside load_metric_config()
...
def _load_phase_config(
    ...
    if canonical_path.exists():
        ...
        print(f"[INFO] Loading phase configuration from central config: {config_path}")
...
def _load_target_config(
    ...
    if "EffectiveFrom" in cfg.columns:
        s = cfg["EffectiveFrom"].astype(str).str.strip()
        iso_mask = s.str.match(r"^\d{4}-\d{2}-\d{2}$", na=False)

        parsed = pd.Series([pd.NaT] * len(cfg), index=cfg.index, dtype="datetime64[ns]")
        ...
        cfg["EffectiveFrom"] = parsed.dt.normalize()
...
def export_spc_from_csv(
    ...
    try:
        metric_configs = _load_metric_config_with_override(config_dir_path)
    ...
    phase_starts = _load_phase_config(config_dir=config_dir_path, working_dir=working_dir_path, group_cols=group_cols)
