...
def _export_spc_from_csv_impl(
    ...
    quiet=True suppresses ALL stdout/stderr output from this exporter (including from
    helper functions), so the CLI can control messaging cleanly.
    """
    if quiet:
        import contextlib
        import io
        ...
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            return _export_spc_from_csv_impl(
                input_csv=input_csv,
