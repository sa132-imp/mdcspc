...
def main():
    # Path to the stable dataset CSV
    ...
    # Run single-series XmR analysis with same settings as the Excel template
    result = analyse_xmr(
        data=df,
        value_col="Value",
        index_col="Month",
    ...
    print("\n=== Core XmR statistics for test dataset 3 (stable system) ===\n")
    ...
    # Per-point rule behaviour – we expect ALL rules to be False
    cols_to_show = [
        "Value",
        "phase",
        "mean",
    ...
    print("\n=== Per-point output for dataset 3 (expected: no special cause) ===\n")
